...
def main() -> None:
    ...
    expected = {
        ...
        'tactical_allocator_explanatory_version': 'v3.4.1',
        'equity_execution_rule': 'V82.md',
        'bond_execution_rule': '債券V75.md',
        'bond_execution_version': 'v2.4.0',
    ...
    # Mentor may not own current-run per-bond weights. BondV75 may own formal
    # long-term bond-sub-bucket target maps, so those are locked in a separate
    # execution-policy section rather than target_weights_within_subbucket.
    ...
    if bond_policy.get('source_file') != '債券V75.md' or bond_policy.get('source_version') != 'v2.4.0':
    ...
    print('V70 output contract: v1.1.1; V70 explanatory spec: v3.4.1')
    print('Equity examples:', ','.join(EXPECTED_EQUITY))
    print('Primary bond examples:', ','.join(EXPECTED_BOND_PRIMARY))
    print('BondV75 v2.4 targets locked separately from Mentor ownership')
